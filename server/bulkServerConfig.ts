...
export const LINUX_SERVER_TEMPLATES: BulkServerTemplate[] = [
  ...
  // 23. Directory Lifecycle, Retention & Fleet Backup Policy
  {
    id: 'linux_directory_lifecycle_backup',
    category: 'storage',
    title: 'مدیریت چرخه حیات دایرکتوری و بکاپ ناوگان (Directory Lifecycle & Fleet Backup)',
    title_en: 'Directory Lifecycle, Retention & Fleet Backup Policy',
    description: 'فشرده‌سازی و بکاپ ایمن با تضمین حفظ کامل بکاپ‌های پیشین، پاک‌سازی فایل‌های قدیمی بر اساس سن، سقف حجم و همگام‌سازی دایرکتوری',
    description_en: 'Automated directory compression & safe backup with overwrite-protection, retention purge, size quota cap, and sync.',
    icon: 'FolderArchive',
    is_dangerous: false,
    confirmation_keyword: '',
    default_timeout_sec: 120,
    info_what_fa: 'مدیریت یکپارچه سیاست‌های فشرده‌سازی بکاپ، پاک‌سازی بر اساس سن فایل، محدودسازی حجم و میرور دایرکتوری‌ها با تضمین عدم حذف یا بازنویسی آرشیوهای قبلی.',
    info_what_en: 'Fleet-wide directory lifecycle management: compressed archives with safe preservation, age-based purge, size cap, and rsync mirror.',
    info_why_fa: 'جلوگیری از انباشت لاگ‌ها و فایل‌های موقت، پشتیبان‌گیری منظم و خودکار از داده‌های حساس و جلوگیری از پر شدن دیسک سرورها.',
    info_why_en: 'Protects critical data via non-overwriting backup archives while actively preventing disk exhaustion.',
    info_example_fa: 'آرشیو روزانه دایرکتوری /var/log/nginx در مسیر /backup/archives با فرمت tar.gz و حفظ تمام نسخه‌های قبلی.',
    info_example_en: 'Daily compressed tar.gz backup of /var/log/app to /backup/archives preserving all prior snapshots.',
    parameters: [
      {
        name: 'action',
        labelFa: 'نوع عملیات سیاست (Policy Action)',
        labelEn: 'Policy Action Type',
        type: 'select',
        required: true,
        default: 'backup',
        options: [
          { value: 'backup', labelFa: 'فشرده‌سازی و آرشیو بکاپ (Compress & Backup)', labelEn: 'Compress & Backup Archive' },
          { value: 'cleanup', labelFa: 'پاک‌سازی فایل‌های قدیمی بر اساس سن (Retention Purge)', labelEn: 'Retention Purge Old Files' },
          { value: 'size_cap', labelFa: 'مهار حجم دایرکتوری با حذف قدیمی‌ترین‌ها (Size Cap Quota)', labelEn: 'Size-Capped Storage Quota' },
          { value: 'sync', labelFa: 'همگام‌سازی و کلون به مسیر دیگر (Directory Sync / Mirror)', labelEn: 'Directory Sync / Mirror' }
        ],
        info_what_fa: 'نوع رفتاری که سیاست چرخه حیات روی دایرکتوری هدف اعمال می‌کند.',
        info_what_en: 'The operational mode to execute on the targeted directory.',
        info_why_fa: 'تفکیک دقیق بین پشتیبان‌گیری، پاک‌سازی یا همگام‌سازی.',
        info_why_en: 'Directs whether to archive, purge, enforce quota, or mirror.',
        info_example_fa: 'backup',
        info_example_en: 'backup'
      },
      {
        name: 'target_path',
        labelFa: 'مسیر دایرکتوری هدف در سرور (Target Directory Path)',
        labelEn: 'Target Directory Path',
        type: 'string',
        required: true,
        default: '/var/log',
        placeholder: '/var/log, /opt/data, /var/backups',
        info_what_fa: 'آدرس کامل دایرکتوری که عملیات چرخه حیات روی آن اجرا می‌شود.',
        info_what_en: 'Absolute path of the directory on target Linux servers.',
        info_why_fa: 'تعریف محدوده اجرای اسکریپت در سیستم‌عامل.',
        info_why_en: 'Specifies the directory scope for execution.',
        info_example_fa: '/var/log/nginx یا /opt/data',
        info_example_en: '/var/log/nginx or /opt/data'
      },
      {
        name: 'schedule_mode',
        labelFa: 'حالت اجرا یا زمان‌بندی (Execution Mode)',
        labelEn: 'Execution / Schedule Mode',
        type: 'select',
        required: true,
        default: 'run_once',
        options: [
          { value: 'run_once', labelFa: 'اجرای فوری در یک نوبت (Run Immediately Once)', labelEn: 'Run Immediately Once' },
          { value: 'daily', labelFa: 'اجرا و زمان‌بندی روزانه رأس ساعت ۰۲:۰۰ (Daily at 02:00 AM)', labelEn: 'Execute & Schedule Daily at 02:00 AM' },
          { value: 'weekly', labelFa: 'اجرا و زمان‌بندی هفتگی یکشنبه‌ها رأس ۰۳:۰۰ (Weekly on Sunday at 03:00 AM)', labelEn: 'Execute & Schedule Weekly Sunday at 03:00 AM' },
          { value: 'hourly', labelFa: 'اجرا و زمان‌بندی ساعتی رأس دقیقه ۰ (Hourly at Minute 0)', labelEn: 'Execute & Schedule Hourly' }
        ],
        info_what_fa: 'تعیین می‌کند که آیا عملیات تنها یک‌بار فوری اجرا شود یا در جدول زمان‌بندی Crontab سرور هم ثبت گردد.',
        info_what_en: 'Controls whether to run once immediately or also register in root crontab.',
        info_why_fa: 'امکان اتوماسیون مداوم بدون نیاز به تعریف جداگانه کرون‌جاب.',
        info_why_en: 'Automates persistent recurring policy runs across the fleet.',
        info_example_fa: 'اجرای فوری یا روزانه',
        info_example_en: 'Run Immediately Once or Daily'
      },
      {
        name: 'backup_format',
        labelFa: 'فرمت فشرده‌سازی بکاپ (Backup Archive Format)',
        labelEn: 'Backup Archive Format',
        type: 'select',
        required: false,
        default: 'tar.gz',
        options: [
          { value: 'tar.gz', labelFa: 'Gzip Tarball (.tar.gz - استاندارد و بهینه)', labelEn: 'Gzip Tarball (.tar.gz - Standard & Fast)' },
          { value: 'tar.bz2', labelFa: 'Bzip2 Tarball (.tar.bz2 - فشرده‌سازی بالا)', labelEn: 'Bzip2 Tarball (.tar.bz2 - Higher Compression)' },
          { value: 'tar.xz', labelFa: 'XZ Tarball (.tar.xz - بالاترین ضریب تراکم)', labelEn: 'XZ Tarball (.tar.xz - Maximum Compression)' },
          { value: 'zip', labelFa: 'آرشیو زیپ استاندارد (.zip)', labelEn: 'Standard ZIP Archive (.zip)' }
        ],
        info_what_fa: 'الگوریتم و پسوند فشرده‌سازی برای تولید آرشیوهای پشتیبان.',
        info_what_en: 'Compression algorithm and extension for generated archives.',
        info_why_fa: 'تعادل بین سرعت پردازش پردازنده و میزان آزادسازی فضای ذخیره‌سازی.',
        info_why_en: 'Balances CPU consumption against disk space savings.',
        info_example_fa: 'tar.gz (پیشنهادی)',
        info_example_en: 'tar.gz (recommended)'
      },
      {
        name: 'backup_destination_path',
        labelFa: 'مسیر ذخیره‌سازی آرشیوهای بکاپ (Backup Destination)',
        labelEn: 'Backup Destination Directory',
        type: 'string',
        required: false,
        default: '/backup/archives',
        placeholder: '/backup/archives, /var/backups',
        info_what_fa: 'پوشه‌ای در سرور که فایل‌های آرشیو تولیدشده در آن قرار می‌گیرند.',
        info_what_en: 'Destination folder on target hosts where backup archives are stored.',
        info_why_fa: 'تفکیک داده‌های اصلی از فایل‌های پشتیبان جهت امنیت و ایزوله‌سازی.',
        info_why_en: 'Separates live production files from archive snapshots.',
        info_example_fa: '/backup/archives',
        info_example_en: '/backup/archives'
      },
      {
        name: 'backup_keep_source_files',
        labelFa: 'حفظ فایل‌های مبدا پس از بکاپ (Keep Source Files)',
        labelEn: 'Keep Source Files After Backup',
        type: 'boolean',
        required: false,
        default: true,
        info_what_fa: 'در صورت فعال بودن، محتویات دایرکتوری مبدا پس از ایجاد آرشیو دست‌نخورده باقی می‌ماند.',
        info_what_en: 'If enabled, source files are preserved after archive generation.',
        info_why_fa: 'جلوگیری از دست رفتن داده‌های در حال استفاده سرویس‌ها.',
        info_why_en: 'Prevents disrupting active services reading from the source path.',
        info_example_fa: 'فعال (True)',
        info_example_en: 'Enabled (True)'
      },
      {
        name: 'backup_preserve_all',
        labelFa: 'حفظ تمام بکاپ‌های قبلی و عدم بازنویسی (Preserve All Existing Backups - Safe Mode)',
        labelEn: 'Preserve All Existing Backups (Safe Mode - No Overwrite)',
        type: 'boolean',
        required: false,
        default: true,
        info_what_fa: 'تضمین می‌کند که بکاپ‌های پیشین موجود در پوشه مقصد دست‌نخورده باقی مانده و هیچ آرشیوی بازنویسی یا پاک نمی‌شود.',
        info_what_en: 'Ensures all prior archives in destination remain intact; archives are sequentially timestamped without overwrite or deletion.',
        info_why_fa: 'جلوگیری قطعی از پاک شدن یا رونویسی آرشیوهای قبلی در زمان گرفتن اسنپ‌شات‌های جدید.',
        info_why_en: 'Guarantees that historical snapshots are never lost when new backups run.',
        info_example_fa: 'فعال (حالت امن بدون حذف)',
        info_example_en: 'Enabled (Safe Preservation Mode)'
      },
      {
        name: 'backup_max_count',
        labelFa: 'سقف نگهداری در صورت چرخش خودکار (Max Retained If Auto-Rotate)',
        labelEn: 'Max Retained Archives (If Auto-Rotate Active)',
        type: 'number',
        required: false,
        default: 7,
        info_what_fa: 'در صورتی که گزینه حفظ تمام بکاپ‌ها غیرفعال باشد، فقط این تعداد از نسخه‌های اخیر نگهداری می‌شوند.',
        info_what_en: 'Active only when Preserve All is disabled; older archives beyond this number will be rotated.',
        info_why_fa: 'جلوگیری از پر شدن فضای دیسک با انباشت صدها آرشیو سال‌های گذشته.',
        info_why_en: 'Prevents destination partition from overflowing when auto-rotation is preferred.',
        info_example_fa: '7 (یک هفته اخیر) یا 14',
        info_example_en: '7 (last 7 archives) or 14'
      },
      {
        name: 'retention_days',
        labelFa: 'مهلت نگهداری فایل‌ها بر حسب روز (Retention Days for Cleanup)',
        labelEn: 'Retention Threshold Days (for Cleanup)',
        type: 'number',
        required: false,
        default: 30,
        info_what_fa: 'تعداد روزهایی که فایل‌ها حفظ می‌شوند؛ فایل‌های قدیمی‌تر از این سقف در حالت cleanup حذف می‌شوند.',
        info_what_en: 'Age threshold in days for the cleanup action; older files matching pattern are purged.',
        info_why_fa: 'آزادسازی خودکار فضای دیسک از فایل‌های لاگ یا دیتای تاریخ‌گذشته.',
        info_why_en: 'Frees disk space occupied by obsolete log files or stale data.',
        info_example_fa: '30 روز یا 90 روز',
        info_example_en: '30 or 90 days'
      },
      {
        name: 'file_pattern',
        labelFa: 'الگوی نام فایل‌ها جهت پاک‌سازی (File Pattern)',
        labelEn: 'File Pattern (*, *.log, *.tmp)',
        type: 'string',
        required: false,
        default: '*',
        placeholder: '*, *.log, *.tmp, *.gz',
        info_what_fa: 'الگوی تطبیق نام فایل‌ها (Glob Pattern) که در عملیات cleanup جستجو و حذف می‌شوند.',
        info_what_en: 'Filename glob pattern for matched deletion in cleanup mode.',
        info_why_fa: 'امکان هدف‌گیری فایل‌های خاص (مانند *.log) بدون لمس سایر فایل‌های دایرکتوری.',
        info_why_en: 'Enables granular filtering of specific file extensions.',
        info_example_fa: '*.log یا *.tmp',
        info_example_en: '*.log or *.tmp'
      },
      {
        name: 'size_cap_mb',
        labelFa: 'سقف مجاز حجم دایرکتوری بر حسب مگابایت (Size Quota Cap in MB)',
        labelEn: 'Size Quota Cap in Megabytes (MB)',
        type: 'number',
        required: false,
        default: 1024,
        info_what_fa: 'حداکثر حجم مجاز دایرکتوری بر حسب مگابایت در حالت size_cap.',
        info_what_en: 'Maximum directory storage ceiling in MB for size_cap enforcement.',
        info_why_fa: 'مهار قطعی مصرف دیسک و جلوگیری از پر شدن کامل پارتیشن.',
        info_why_en: 'Caps runaway disk growth by automatically pruning oldest files.',
        info_example_fa: '1024 (معادل ۱ گیگابایت) یا 5120',
        info_example_en: '1024 (1 GB) or 5120 (5 GB)'
      },
      {
        name: 'sync_destination_path',
        labelFa: 'مسیر مقصد همگام‌سازی (Sync Destination Path)',
        labelEn: 'Sync Destination Directory',
        type: 'string',
        required: false,
        default: '/backup/mirrors',
        placeholder: '/backup/mirrors, /mnt/nfs/mirror',
        info_what_fa: 'مسیر مقصد در سرور که دایرکتوری مبدا به آن منتقل یا میرور می‌شود.',
        info_what_en: 'Target mirror directory for the sync action.',
        info_why_fa: 'ایجاد نسخه پشتیبان آینه‌ای از فایل‌های دایرکتوری در پارتیشن یا دیسک دیگر.',
        info_why_en: 'Creates a live directory replica on another disk/partition.',
        info_example_fa: '/backup/mirrors/app',
        info_example_en: '/backup/mirrors/app'
      },
      {
        name: 'sync_delete_extraneous',
        labelFa: 'حذف فایل‌های اضافه در مقصد همگام‌سازی (Sync Delete Extraneous)',
        labelEn: 'Sync Delete Extraneous Files (--delete)',
        type: 'boolean',
        required: false,
        default: false,
        info_what_fa: 'حذف فایل‌هایی در دایرکتوری مقصد که دیگر در مبدا وجود ندارند (مشابه آرگومان --delete در rsync).',
        info_what_en: 'Removes files in destination that no longer exist in source path (rsync --delete).',
        info_why_fa: 'یکسان‌سازی ۱۰۰٪ و مطابقت دقیق دایرکتوری مقصد با مبدا.',
        info_why_en: 'Keeps mirror directory exactly identical to source.',
        info_example_fa: 'غیرفعال (False) برای امنیت بیشتر',
        info_example_en: 'Disabled (False) for extra safety'
      }
    ],
    distro_commands: {
      generic: [
        [
          ...
          'SCHED="{{schedule_mode}}"',
          'B_FMT="{{backup_format}}"',
          'B_DEST="{{backup_destination_path}}"',
          'B_KEEP="{{backup_keep_source_files}}"',
          'B_PRESERVE="{{backup_preserve_all}}"',
          'B_MAX="{{backup_max_count}}"',
          'RET_DAYS="{{retention_days}}"',
          'F_PAT="{{file_pattern}}"',
          'CAP_MB="{{size_cap_mb}}"',
          'S_DEST="{{sync_destination_path}}"',
          'S_DEL="{{sync_delete_extraneous}}"',
          'echo "=== Directory Lifecycle & Fleet Policy Execution ==="',
          'echo "Target: $TARGET | Action: $ACTION | Schedule: $SCHED"',
          'if [ ! -e "$TARGET" ]; then echo "ERROR: Target path does not exist: $TARGET" >&2; exit 1; fi',
          'case "$ACTION" in',
          '  backup)',
          '    bdest="${B_DEST:-/backup/archives}"',
          '    mkdir -p "$bdest"',
          '    base_name=$(basename "$TARGET")',
          '    ts=$(date "+%Y%m%d_%H%M%S")',
          '    case "$B_FMT" in tar.bz2) ext="tar.bz2" ;; tar.xz) ext="tar.xz" ;; zip) ext="zip" ;; *) ext="tar.gz" ;; esac',
          '    archive_file="$bdest/${base_name}_${ts}.${ext}"',
          '    seq=0',
          '    while [ -e "$archive_file" ]; do seq=$((seq + 1)); archive_file="$bdest/${base_name}_${ts}_${seq}.${ext}"; done',
          '    echo "[BACKUP] Creating archive $archive_file from $TARGET ..."',
          '    case "$B_FMT" in',
          '      tar.bz2) tar -cjf "$archive_file" -C "$(dirname "$TARGET")" "$base_name" 2>&1 ;;',
          '      tar.xz) tar -cJf "$archive_file" -C "$(dirname "$TARGET")" "$base_name" 2>&1 ;;',
          '      zip) (cd "$(dirname "$TARGET")" && zip -rq "$archive_file" "$base_name") 2>&1 ;;',
          '      *) tar -czf "$archive_file" -C "$(dirname "$TARGET")" "$base_name" 2>&1 ;;',
          '    esac',
          '    if [ ! -f "$archive_file" ]; then echo "ERROR: Backup archive creation failed" >&2; exit 1; fi',
          '    arch_size=$(du -sh "$archive_file" | cut -f1)',
          '    echo "[SUCCESS] Archive created: $(basename "$archive_file") ($arch_size)"',
          '    if [ "$B_KEEP" = "false" ] || [ "$B_KEEP" = "0" ]; then',
          '      if [ -d "$TARGET" ]; then find "$TARGET" -mindepth 1 -delete 2>/dev/null || true; echo "[SOURCE] Purged source directory contents (folder kept)."; fi',
          '    fi',
          '    if [ "$B_PRESERVE" = "false" ] || [ "$B_PRESERVE" = "0" ]; then',
          '      if [ -n "$B_MAX" ] && [ "$B_MAX" -gt 0 ]; then',
          '        total_archives=$(ls -1t "$bdest/${base_name}_"* 2>/dev/null | grep -E "\\.(tar\\.(gz|bz2|xz)|zip)$" | wc -l)',
          '        if [ "$total_archives" -gt "$B_MAX" ]; then',
          '          prune_count=$((total_archives - B_MAX))',
          '          ls -1t "$bdest/${base_name}_"* 2>/dev/null | grep -E "\\.(tar\\.(gz|bz2|xz)|zip)$" | tail -n "$prune_count" | xargs -r rm -f',
          '          echo "[RETENTION] Pruned $prune_count old archives exceeding auto-rotate limit of $B_MAX."',
          '        fi',
          '      fi',
          '    else',
          '      echo "[RETENTION] Safe preservation mode: All previous and existing backups in $bdest remain completely intact."',
          '    fi',
          '    ;;',
          '  cleanup)',
          '    days="${RET_DAYS:-30}"',
          '    pat="${F_PAT:-*}"',
          '    echo "[CLEANUP] Purging files older than $days days matching \"$pat\" in $TARGET ..."',
          '    match_count=$(find "$TARGET" -mindepth 1 -type f -name "$pat" -mtime +"$days" 2>/dev/null | wc -l)',
          '    find "$TARGET" -mindepth 1 -type f -name "$pat" -mtime +"$days" -delete 2>/dev/null || true',
          '    echo "[SUCCESS] Purged $match_count stale files exceeding $days days."',
          '    ;;',
          '  size_cap)',
          '    cap="${CAP_MB:-1024}"',
          '    echo "[SIZE_CAP] Checking size of $TARGET against ${cap} MB quota ..."',
          '    curr_mb=$(du -sm "$TARGET" 2>/dev/null | cut -f1)',
          '    echo "Current size: ${curr_mb} MB | Allowed limit: ${cap} MB"',
          '    if [ "$curr_mb" -gt "$cap" ]; then',
          '      echo "Target exceeds quota by $((curr_mb - cap)) MB. Pruning oldest files..."',
          '      while [ $(du -sm "$TARGET" 2>/dev/null | cut -f1) -gt "$cap" ]; do',
          '        oldest=$(find "$TARGET" -mindepth 1 -type f -printf "%T@ %p\\n" 2>/dev/null | sort -n | head -n 1 | cut -d" " -f2-)',
          '        [ -z "$oldest" ] && break',
          '        rm -f "$oldest"',
          '      done',
          '      new_mb=$(du -sm "$TARGET" 2>/dev/null | cut -f1)',
          '      echo "[SUCCESS] Reduced directory size from ${curr_mb} MB to ${new_mb} MB."',
          '    else',
          '      echo "[OK] Directory size is within allowed quota."',
          '    fi',
          '    ;;',
          '  sync)',
          '    sdest="${S_DEST:-/backup/mirrors}"',
          '    mkdir -p "$sdest"',
          '    echo "[SYNC] Syncing $TARGET to $sdest ..."',
          '    del_arg=""',
          '    if [ "$S_DEL" = "true" ] || [ "$S_DEL" = "1" ]; then del_arg="--delete"; fi',
          '    if command -v rsync >/dev/null 2>&1; then rsync -avz $del_arg "$TARGET/" "$sdest/" | tail -n 5; else cp -a "$TARGET/." "$sdest/"; fi',
          '    echo "[SUCCESS] Sync completed to $sdest."',
          '    ;;',
          '  *)',
          '    echo "ERROR: Unknown action: $ACTION" >&2; exit 1',
          '    ;;',
          'esac',
          'if [ "$SCHED" != "run_once" ]; then',
          '  case "$SCHED" in hourly) CRON_EXPR="0 * * * *" ;; daily) CRON_EXPR="0 2 * * *" ;; weekly) CRON_EXPR="0 3 * * 0" ;; *) CRON_EXPR="0 2 * * *" ;; esac',
          '  TAG="# NETMGMT_LIFECYCLE_$(echo "$TARGET" | tr "/" "_")"',
          '  CRON_LINE="$CRON_EXPR /bin/bash -c \\"TARGET=\'$TARGET\' ACTION=\'$ACTION\' B_FMT=\'$B_FMT\' B_DEST=\'$B_DEST\' B_KEEP=\'$B_KEEP\' B_PRESERVE=\'$B_PRESERVE\' B_MAX=\'$B_MAX\' RET_DAYS=\'$RET_DAYS\' F_PAT=\'$F_PAT\' CAP_MB=\'$CAP_MB\' S_DEST=\'$S_DEST\' S_DEL=\'$S_DEL\' [ -e \'$TARGET\' ] && ( [ \'$ACTION\' = \'backup\' ] && mkdir -p \'$B_DEST\' && tar -czf \'$B_DEST/\\$(basename \'$TARGET\')_\\$(date +\\%Y\\%m\\%d_\\%H\\%M\\%S).tar.gz -C \\$(dirname \'$TARGET\') \\$(basename \'$TARGET\') 2>/dev/null || true )\\" >> /var/log/netmgmt-policy.log 2>&1 $TAG"',
          '  EXISTING=$(crontab -l 2>/dev/null || true)',
          '  CLEANED=$(echo "$EXISTING" | grep -v "$TAG" || true)',
          '  TMP_CRON=$(mktemp)',
          '  if [ -n "$CLEANED" ]; then echo "$CLEANED" > "$TMP_CRON"; echo "$CRON_LINE" >> "$TMP_CRON"; else echo "$CRON_LINE" > "$TMP_CRON"; fi',
          '  crontab "$TMP_CRON"',
          '  rm -f "$TMP_CRON"',
          '  echo "[SCHEDULE] Automated Crontab schedule configured: $CRON_EXPR $TAG"',
          'fi'
        ].join('; ')
      ]
    }
