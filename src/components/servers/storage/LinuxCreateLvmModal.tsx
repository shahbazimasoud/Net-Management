...
export const LinuxCreateLvmModal: React.FC<LinuxCreateLvmModalProps> = ({
  ...
  useEffect(() => {
    if (allVgs.length > 0 && !existingVgName) {
      ...
      if (res.success) {
        setFeedback({
          ...
          message:
            res.message ||
            (isEn
              ? 'Logical Volume created and formatted successfully!'
              : 'فضای لاجیکال ولوم با موفقیت ساخته، فرمت و مانت شد!'),
        });
        setTimeout(() => {
          onSuccess();
          onClose();
