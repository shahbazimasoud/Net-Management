...
export const LinuxUsersTab: React.FC<LinuxUsersTabProps> = ({
  ...
  const [userForLogout, setUserForLogout] = useState<LinuxLoggedInUser | null>(null);
  ...
  return (
    <div className="space-y-6">
                    ...
                    <td className="py-2.5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          title={isEn ? 'Send terminal message' : 'ارسال پیام به ترمینال'}
                          className="px-2 py-1 rounded bg-cyan-500/15 hover:bg-cyan-500/25 text-cyan-400 border border-cyan-500/30 text-[11px] font-bold transition cursor-pointer flex items-center gap-1"
                        >
                          <MessageSquare className="w-3 h-3" />
                          <span>{isEn ? 'Message' : 'پیام'}</span>
                        </button>
                        <button
                          type="button"
                          onClick={() => setUserForLogout(u)}
                          title={isEn ? 'Log out / terminate session' : 'قطع اتصال و خروج نشست'}
                          className="px-2 py-1 rounded bg-rose-500/15 hover:bg-rose-500/25 text-rose-400 border border-rose-500/30 text-[11px] font-bold transition cursor-pointer flex items-center gap-1"
                        >
                          <LogOut className="w-3 h-3" />
                          <span>{isEn ? 'Log Out' : 'خروج'}</span>
                        </button>
                      </div>
                    </td>
                  </tr>
      ...
      {/* Logout User Session Modal */}
      {userForLogout && (
        <LogoutUserSessionModal
          server={server}
          userSession={userForLogout}
          ephemeralPassword={ephemeralPassword}
          isLightMode={isLightMode}
          isEn={isEn}
          onClose={() => setUserForLogout(null)}
          onSuccess={(msg) => {
            showNotification(msg);
            loadUserData();
          }}
