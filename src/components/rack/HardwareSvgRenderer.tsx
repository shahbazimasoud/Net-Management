...
export const HardwareSvgRenderer: React.FC<HardwareSvgRendererProps> = ({
  ...
  const safeCards = Array.isArray(device.networkCards) ? device.networkCards : [];
  const totalPorts = safeCards.reduce((acc, c) => acc + (c.portCount || 0), 0);
        ...
        <g transform="translate(220, 3)">
          ...
          <g transform="translate(4, 9)">
            {safeCards.map((card, cIdx) => {
              const cardWidth = Math.max(48, Math.floor((mainW - 232) / Math.max(1, safeCards.length)));
