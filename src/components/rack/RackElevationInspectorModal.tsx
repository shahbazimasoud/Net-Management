          ...
          <div className="lg:col-span-5 h-full overflow-y-auto p-6 space-y-6 bg-slate-950">
            ...
            {currentSelectedDevice ? (
              <div className="p-5 rounded-2xl bg-slate-900 border border-cyan-500/50 space-y-4 shadow-xl">
                ...
                {(() => {
                  const safeNics = Array.isArray(currentSelectedDevice.networkCards) ? currentSelectedDevice.networkCards : [];
                  return (
                    <div className="space-y-2 pt-2 border-t border-slate-800">
                      <span className="text-xs font-bold text-slate-300 block">
                        {isEn
                          ? `Network Cards & Interfaces (${safeNics.length} NICs):`
                          : `کارت‌های شبکه و اینترفیس‌ها (${safeNics.length} کارت):`}
                      </span>
                      <div className="space-y-2">
                        {safeNics.map((card, idx) => (
                          <div
                            key={card.id || idx}
              ...
              {safeRack.devices.length === 0 ? (
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {safeRack.devices.map((dev) => {
                    ...
                    const devPorts = (dev.networkCards || []).reduce((acc, c) => acc + (c.portCount || 0), 0);
                    return (
                      <div
                        key={dev.id}
