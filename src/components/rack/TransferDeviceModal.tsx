...
export const TransferDeviceModal: React.FC<TransferDeviceModalProps> = ({
  ...
  return createPortal(
    <div
      className="fixed top-0 left-0 right-0 bottom-8 z-[1100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in"
      dir={isRtl ? 'rtl' : 'ltr'}
    >
      <div
        ...
        {otherRacks.length === 0 ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="space-y-2">
              ...
              >
                {otherRacks.map((r) => {
                  const rDevs = Array.isArray(r.devices) ? r.devices : [];
                  const used = rDevs.reduce((acc, d) => acc + d.heightU, 0);
                  const free = r.units - used;
                  return (
                    <option key={r.id} value={r.id}>
                      {r.name} ({r.units}U • {free}U {isEn ? 'free' : 'آزاد'} • {rDevs.length} {isEn ? 'devices' : 'تجهیز'})
                    </option>
                  );
                })}
              </select>
