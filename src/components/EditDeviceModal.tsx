...
import { Device, DeviceType, DevicePlatform, ConnectionMode, SwitchPort, DeviceWebConfig } from '../types';
...
export const EditDeviceModal: React.FC<EditDeviceModalProps> = ({
  ...
  useEffect(() => {
    ...
    setWebConfigs(Array.isArray(device.web_configs) ? device.web_configs.map((w) => ({ ...w })) : []);
      ...
      fetchDevicePorts(device.id)
        .then((res) => {
          if (res && res.ports && res.ports.length > 0) {
            ...
            if (res.ports.length > 0 && !device.total_ports) {
